...
class Compilor_Parser_Pager
{
    ...
    function main()
    {
        $tags = $this->parser->SearchTag('pager');
        $this->tags = $tags;
        foreach ($tags as $t) {
            $this->GCode($t['properties']['name'], 'Pager', $t['properties']);
            $name = $t['properties']['name'];
            $this->parser->ReplaceTag($t['former_str'], '<?php $components["Pager_' . $name . '"]->OutPutContent();?>');
...
class {$basename}_$name extends Component$basename{";
        ...
        $code_body['functions']['__construct'] = "\n\tfunction __construct(){" .
            ...
            "\$components['{$properties['target']}']->start = (int)\$_GET[\$this->flag];";
